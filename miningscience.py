...
def download_pubmed(keyword): 
    """Descarga la base de datos y las guarda en un archivo txt.
    """
...
def map_science (tipo):
    """se crea un grafico para ciudades elegidas por medio de una base de datos, la cual fue colocada anteriormente"""
    #if tipo == "AD":
    with open(tipo) as f:
        my_text = f.read()
    my_text = re.sub(r'\n\s{6}', ' ', my_text)  
...
# add a few cities for reference (optional)
    ard = dict(arrowstyle="->")
    plt.annotate('Holanda', xy = (-118.25, 34.05), 
                   xytext = (-108.25, 34.05), arrowprops = ard)
    plt.annotate('Egypto', xy = (-122.1381, 37.4292), 
                   xytext = (-112.1381, 37.4292), arrowprops= ard)
    plt.annotate('Colombia', xy = (-71.1106, 42.3736), 
                   xytext = (-73.1106, 48.3736), arrowprops= ard)
    plt.annotate('Brasil', xy = (-87.6847, 41.8369), 
                   xytext = (-87.6847, 46.8369), arrowprops= ard)
    plt.annotate('Ahiti', xy = (-122.33, 47.61), 
                   xytext = (-116.33, 47.61), arrowprops= ard)
    plt.annotate('Vietnam', xy = (-80.21, 25.7753), 
